...
5. **Live credentials available?** Yes. The key is present in the environment under `THESTATS_API_KEY` (the deploy/cron name); the code accepts it as an alias of the documented `THESTATSAPI_API_KEY` via `resolve_api_key()`. The value is never printed, logged, or committed.
6. **Live endpoints actually exercised.** `/health` (200) and `/football/matches?status=scheduled&competition_id=comp_3039` (200) — minimal read-only smoke only.
7. **Sanitized live response-shape findings.** Match list returns stable `id`/`utc_date`/`home_team.id`/`away_team.id`; critical-field schema validates (no drift). Upcoming EPL fixtures showed `odds_available=false` this far from kickoff (expected, not an error).
...
15. **Rate-limit/quota findings.** Two budgets parsed; confirmed LIVE on an authenticated `/football/matches` response: minute limit **120** (rem 119), monthly limit **100,000** (rem 97,400). Conservative pause on low/unknown/exhausted; honours `Retry-After`.
...
33. **LIVE_CAPTURE readiness.** **LIVE_CAPTURE_READY** — key available (via `THESTATS_API_KEY` alias), auth + base URL + budgets + match schema validated live; `capture-due` can run against the initial universe.

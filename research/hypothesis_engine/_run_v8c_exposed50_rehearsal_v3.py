"""V8C exposed-50 COMPOSED rehearsal (`v8c_exposed50_rehearsal_v3`).
...
OUT = os.environ.get("V8C_REHEARSAL_OUT", f"{ENG}/V8C_EXPOSED50_REHEARSAL_V3.json")
ARCHIVE = "/home/ubuntu/handoff_out/V8C_DEV_EXPOSED50_HANDOFF_CORRECTED.tar.gz"
...
def prevalidate_bundle(bundle_root: str) -> dict:
    """REPAIR 2. The bundle is validated against the EXTERNALLY PINNED ARCHIVE DIGEST, with
    the approved file list read from inside that verified archive, season files scanned for
    sealed ids, and every loader root bound to the one validated directory.

    `bundle_root` here is the directory the archive is extracted INTO.
    """
    from src.research.hypothesis_v8c import bundle_gate as BG
    return BG.open_bundle(ARCHIVE, bundle_root)


def _apparatus(bundle_root):
    """`bundle_root` is the VALIDATED bundle directory returned by the gate."""
    sys.path.insert(0, os.path.join(bundle_root, "loader_adapter"))
    import v8c_dev_paths
    v8c_dev_paths.CACHE_ROOT = os.path.join(bundle_root, "data", "thestatsapi", "championship")
...
def process_1(bundle_root, outdir, k=4, n_limit=None):
    ...
    validated = pre["bundle_root"]
    paths, cap, index, n_recs = _apparatus(validated)
    ...
    from src.research.hypothesis_v8c import pit_context as _PC
    _HS_STATS[0] = None
    try:
        _hs = _PC.build_pit_context(index, positions[0]).historical_similarity
        _HS_STATS[0] = _hs.cache_stats()
    except Exception:
        pass
    ...
    json.dump({"phase": "PROCESS_1_COMPLETE", "prevalidation": pre,
               "peak_rss_mb": _rss_mb(),
               "similarity_cache_stats": _HS_STATS[0],
...
def process_2(bundle_root, outdir, anchor_commit, anchor_rel, repo_root):
    ...
    pre = prevalidate_bundle(bundle_root)
    _paths, cap, index, n_recs = _apparatus(pre["bundle_root"])
    fp = os.path.join(outdir, "freeze.json")
    rp = os.path.join(outdir, "freeze_receipt.json")
    ...
    res = SFZ.score_frozen(fp, index, capability=cap, receipt_path=rp,
                           ...
                           mode=SFZ.DEVELOPMENT_DIAGNOSTICS,
                           expected_exec_root=os.environ.get("V8C_EXEC_ROOT"))
    ...
    out = {"phase": "PROCESS_2_COMPLETE",
           ...
           "n_sources_required": res["n_sources_required"],
           "n_executing_verified": res["n_executing_verified"],
           "structural_diagnostics": res["structural_diagnostics"],
           "effect_aggregation_called": res["effect_aggregation_called"],
           "statistical_inference_called": res["statistical_inference_called"],
           "arm_score_ok": res["structural_diagnostics"]["arm_score_ok"],
           "n_records_scored": res["structural_diagnostics"]["n_records"],
           "elapsed_s": round(time.time() - t0, 1)}
...
def run(bundle_root, outdir, k=4, n_limit=None):
    ...
    report = {
        "rehearsal_version": "v8c_exposed50_rehearsal_v3",
        ...
        "OUTCOME_SEAL_PASS": (p1["prevalidation"]["seal"]["sealed_in_stats_files"] == 0
                              and p1["prevalidation"]["seal"]["sealed_in_season_files"] == 0),
        "SEASON_FILES_SCANNED_FOR_SEALED_IDS":
            p1["prevalidation"]["season_files_checked"],
        "N_SEASON_FILES_SCANNED": p1["prevalidation"]["n_season_files"],
        "N_APPROVED_FIXTURE_IDS": p1["prevalidation"]["n_approved_fixture_ids"],
        "ARCHIVE_DIGEST_VERIFIED": p1["prevalidation"]["archive"]["archive_sha256"],
        "BUNDLE_TREE_FILES_VERIFIED": p1["prevalidation"]["tree"]["n_files_verified"],
        "OUTCOME_SEAL_IS_STRUCTURAL_AND_PREVALIDATED": True,
        "ARM_SCORE_OK": p2["arm_score_ok"],
        "N_RECORDS_SCORED": p2["n_records_scored"],
        "STRUCTURAL_DIAGNOSTICS": p2["structural_diagnostics"],
        "EFFECT_AGGREGATION_CALLED": p2["effect_aggregation_called"],
        "STATISTICAL_INFERENCE_CALLED": p2["statistical_inference_called"],
        "N_SOURCES_REQUIRED": p2["n_sources_required"],
        "N_EXECUTING_VERIFIED": p2["n_executing_verified"],

"""Outcome-blind V1.2 support rerun on the exact frozen V1 scoring panel.
...
SUPPORT_PLAN = OUT / "V1_2_SUPPORT_EXECUTION_PLAN_V1.json"
...
PLAN_SHA256 = "39d7a0fa46bda14e742b86de1f703f74b711017158f6164b1fcc83849eccdc0d"
...
def audit_md(gate: dict, history_meta: dict, threshold: float) -> str:
    lines = [
        "# Sol V1.2 Outcome-Blind Evaluability Audit", "",
    ...
    lines += [
        ...
        "A FAIL aborts V1.2 before target construction; no threshold, fold, template, "
...
def main() -> None:
    ...
    diagnostics = {
        "artifact_version": "target_aware_panel_support_v1_2",
    ...
    decision = (
        "PASS_V1_2_EVALUABILITY_GATE"
        if gate_pass else "ABORT_V1_2_BEFORE_OUTCOME_ACCESS"
    )
    gate = {
        "artifact_version": "target_aware_v1_2_evaluability_gate_decision_v1",
        "decision": decision,
        "gate_pass": gate_pass,
        "rule": (
    ...
    manifest = {
        "artifact_version": "sol_panel_support_freeze_manifest_v1_2",
        "authorized_execution_head": head,
        "support_execution_plan_sha256": PLAN_SHA256,
        "prehistory_raw_manifest_sha256":
            plan["prehistory_raw_manifest_sha256"],
        ...
        "champion_changed": False,
        "next_gate": (
            "REVIEW_FROZEN_V1_2_EVALUABILITY_PASS_BEFORE_PREDICTIVE_OOS"
            if gate_pass else "V1_2_ABORTED"
        ),
    }

    manifest_sha = dump(OUT / OUTPUTS[3], manifest)
    print(json.dumps({
        "status": "V1_2_SUPPORT_AND_EVALUABILITY_COMPLETE",
        "decision": decision,
        "gate_pass": gate_pass,
        "diagnostics_sha256": diag_sha,
        "audit_sha256": audit_sha,

...
class StrengthRatings:
    ...
    Processed strictly in kickoff order; the rating available for fixture F reflects
    only matches that kicked off before F. Multiplicative model around the league
    running mean, with shrinkage toward 1.0 by match count.
    """
    def __init__(self, recs: list[MatchRecord], stat: str, shrink: float = 4.0):
        ...
        for r in sorted(recs, key=lambda x: x.kickoff_unix):
            s = season_of(r)
            try:
                hv, av = stat_pair(r, stat)
            except KeyError:
                continue
            # SNAPSHOT (before updating with this match) — what a model at F may use
            ...
            # UPDATE running state with this match (now becomes past for future fixtures)
